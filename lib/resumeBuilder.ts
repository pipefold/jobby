import { JSONResume, InterviewResponse, InterviewSession } from '@/types/resume';
...
export function buildResumeFromInterview(
  ...
  for (const [section, sectionResponses] of Object.entries(responsesBySection)) {
    switch (section) {
      case 'basics':
...
function buildBasicsFromResponses(responses: InterviewResponse[]) {
  ...
  responses.forEach((response) => {
    ...
    } else if (questionLower.includes('summary') || questionLower.includes('about yourself')) {
      basics.summary = response.answer;
    } else if (questionLower.includes('location') || questionLower.includes('city')) {
      if (basics.location) {
...
function buildWorkFromResponses(responses: InterviewResponse[]) {
  ...
  responses.forEach((response, index) => {
    ...
    if (questionLower.includes('company') || questionLower.includes('employer')) {
      if (currentWork.name) {
        work.push(currentWork);
        currentWork = {};
      }
      currentWork.name = response.answer;
    } else if (questionLower.includes('position') || questionLower.includes('title')) {
    ...
    } else if (questionLower.includes('description') || questionLower.includes('responsibilities')) {
      currentWork.summary = response.answer;
    } else if (questionLower.includes('achievements') || questionLower.includes('highlights')) {
      currentWork.highlights = response.answer.split('\n').filter((h: string) => h.trim());
    }
  });
...
function buildEducationFromResponses(responses: InterviewResponse[]) {
  ...
  responses.forEach((response) => {
    ...
    if (questionLower.includes('school') || questionLower.includes('university') || questionLower.includes('institution')) {
      if (currentEdu.institution) {
        education.push(currentEdu);
        currentEdu = {};
      }
      currentEdu.institution = response.answer;
    } else if (questionLower.includes('degree') || questionLower.includes('study type')) {
      currentEdu.studyType = response.answer;
    } else if (questionLower.includes('field') || questionLower.includes('major') || questionLower.includes('area')) {
      currentEdu.area = response.answer;
    } else if (questionLower.includes('start date') || questionLower.includes('graduation')) {
...
function buildProjectsFromResponses(responses: InterviewResponse[]) {
  ...
  responses.forEach((response) => {
    ...
    } else if (questionLower.includes('highlights') || questionLower.includes('achievements')) {
      currentProject.highlights = response.answer.split('\n').filter((h: string) => h.trim());
    } else if (questionLower.includes('url') || questionLower.includes('link')) {
